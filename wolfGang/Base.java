...
public class Base extends MyUnit {
    ...
    void playRound() {
        ...
        if(uc.getResource(Resource.WOOD) > 15 && (!raftsRequested || hasTech(Technology.RAFTS))) {
            ...
            if (((uc.getTotalUnits() < 7 && uc.getRound() < 150) || (((workerCount < 20 + uc.getRound() / 300) || lastWorkerSeenRound < uc.getRound() - 150) && uc.getTotalUnits() <= 45)))
    ...
    void research() {
        ...
        if (techPhase == 0) { // pre-jobs
            ...
            if (hasTech(Technology.UTENSILS) && hasTech(Technology.DOMESTICATION) && (!raftsRequested || hasTech(Technology.RAFTS)) && hasTech(Technology.MILITARY_TRAINING)) // && hasTech(Technology.MILITARY_TRAINING)
                techPhase++;
