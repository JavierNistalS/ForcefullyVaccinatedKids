...
public abstract class MyUnit {
    ...
    Direction[] diags = {Direction.NORTHEAST, Direction.NORTHWEST, Direction.SOUTHEAST, Direction.SOUTHWEST};

    Location baseLocation, enemyBaseLocation, enemyStructureLocation;
    ...
    boolean moveRandomDiagonal(){
        int tries = 10;
        while (uc.canMove() && tries-- > 0){
            int random = (int)(uc.getRandomDouble()*4);
            if (uc.canMove(diags[random])){
                uc.move(diags[random]);
                return true;
            }
        }
        return false;
